import ScuttlebuttPeer from "./ScuttlebuttRpcPeer.ts";
...
const host = new ScuttlebuttPeer(new Procedures());

host.listen();

host.addEventListener("connected", async (options) => {
  log.debug("new connection");
  const rpcConnection: RPCConnection = (options as CustomEvent).detail;
...
await Promise.all(peers.map((address) =>
  (async () => {
    ...
    while (true) {
      try {
        if (host.boxPeer.connections.length > 20) {
          log.info("More than 20 connections open, standing by.");
        } else {
          log.info(
            `${host.boxPeer.connections.length} connections open, connecting to ${address}`,
          );
          await host.connect(address);
        }
      } catch (error) {
        log.error(
          `In connection with ${address}: ${error}, now having ${host.boxPeer.connections.length} connections left`,
        );
